    ...
    R: CryptoRng,
{
    fn with_random_cols(
        &self,
        mut matrix: RowMajorMatrix<Val>,
        extra_cols: usize,
    ) -> RowMajorMatrix<Val> {
        let original_width = matrix.width();
        // Matrix widening must finish before the RNG lock: matrix operations may enter Rayon.
        matrix.widen_right(extra_cols, Val::ZERO);
        let mut rng = self.rng.lock();
        for row in matrix.rows_mut() {
            for value in &mut row[original_width..] {
                *value = rng.random();
            }
        }
        matrix
    ...
    R: CryptoRng + Send + Sync,
{
                ...
                evaluations
                    ...
                    .map(|(domain, mat)| {
                        ...
                        let mut random_evaluation =
                            self.with_random_cols(mat, mat_width + 2 * self.num_random_codewords);
    ...
    fn get_quotient_ldes(
        ...
        let randomized_evaluations: Vec<RowMajorMatrix<Val>> = evaluations
            .into_iter()
            .map(|mat| self.with_random_cols(mat, self.num_random_codewords))
            .collect();
        ...
        let mut all_random_values = (0..(randomized_evaluations.len() - 1) * h * w)
            ...
            .collect::<Vec<_>>();
        // Release the RNG before FFT work can execute another table on this Rayon worker.
        drop(rng);
...
mod tests {
    ...
    #[test]
    fn parallel_quotient_masking_does_not_reenter_rng_lock() {
        use p3_maybe_rayon::prelude::*;

        let (pcs, _, _, _) = make_fixture();
        let domain = <MyPcs as Pcs<Challenge, Challenger>>::natural_domain_for_degree(&pcs, 64);
        (0..64).into_par_iter().for_each(|_| {
            let chunks = domain.split_domains(2);
            let expected_height = chunks[0].size() << 2;
            let evaluations = chunks.into_iter().map(|domain| {
                let matrix =
                    RowMajorMatrix::new((0..domain.size() * 3).map(Val::from_usize).collect(), 3);
                (domain, matrix)
            });
            let ldes =
                <MyPcs as Pcs<Challenge, Challenger>>::get_quotient_ldes(&pcs, evaluations, 2);
            assert_eq!(ldes.len(), 2);
            for lde in ldes {
                assert_eq!(lde.height(), expected_height);
                assert_eq!(lde.width(), 3 + NUM_RANDOM_CODEWORDS);
            }
        });
    }

    #[derive(Clone, Default)]
    struct LockObservingDft {
        before_lde: Option<alloc::sync::Arc<dyn Fn() + Send + Sync>>,
    }

    impl TwoAdicSubgroupDft<Val> for LockObservingDft {
        ...
        fn coset_lde_batch(
            ...
            self.before_lde
                .as_ref()
                .expect("test installs its observation")();
            Dft::default().coset_lde_batch(mat, added_bits, shift)
        }
    }

    #[test]
    fn quotient_lde_releases_rng_before_entering_reentrant_dft() {
        use alloc::sync::Arc;
        use core::sync::atomic::{AtomicUsize, Ordering};

        type ObservedPcs = HidingFriPcs<Val, LockObservingDft, ValMmcs, ChallengeMmcs, StdRng>;
        let mut rng = SmallRng::seed_from_u64(1);
        let perm = Perm::new_from_rng_128(&mut rng);
        let mmcs = ValMmcs::new(MyHash::new(perm.clone()), MyCompress::new(perm), 0);
        ...
        let observed = Arc::new_cyclic(|weak: &alloc::sync::Weak<ObservedPcs>| {
            let weak = weak.clone();
            let calls = calls.clone();
            ObservedPcs::new(
                LockObservingDft {
                    before_lde: Some(Arc::new(move || {
                        let pcs = weak.upgrade().expect("PCS owns the active DFT call");
                        assert!(
                            pcs.rng.try_lock().is_some(),
                            "RNG lock crosses the reentrant DFT boundary"
                        );
        ...
        let domain =
            <MyPcs as Pcs<Challenge, Challenger>>::natural_domain_for_degree(&ordinary, 16);
        let chunks = 2;
        let evaluations = domain
            ...
            .collect::<Vec<_>>();
        let expected = <MyPcs as Pcs<Challenge, Challenger>>::get_quotient_ldes(
            &ordinary,
            evaluations.clone(),
            chunks,
        );
        let actual = <ObservedPcs as Pcs<Challenge, Challenger>>::get_quotient_ldes(
            &observed,
            evaluations,
            chunks,
        );
